...
tree.gini = function(X, y){
  ...
  while (!full.grown){
    ...
    escape_while = escape_while + 1
    NODES = new.nodes
    NODE.record = append(NODE.record, list(NODES))
    full.grown = escape_while > 2
  }
  return(list("nodes" = NODE.record, "splits" = SPLITS))
}
res.tree = tree.gini(X_b, Y)
```

The function defined above, which induced depression during construction, returns only a single split, pictured below. 
